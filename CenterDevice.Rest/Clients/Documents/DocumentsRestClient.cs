...
namespace CenterDevice.Rest.Clients.Documents
{
    public class DocumentsRestClient : CenterDeviceRestClient, IDocumentsRestClient
    {
        ...
        private const string URI_RESOURCE = "v2/documents";

        private readonly StreamWrapper streamWrapper;

        [Inject]
        public DocumentsRestClient(IOAuthInfoProvider oauthInfoProvider, IRestClientConfiguration configuration, IRestClientErrorHandler errorHandler, StreamWrapper streamWrapper) : base(oauthInfoProvider, configuration, errorHandler) { this.streamWrapper = streamWrapper; }
